	...
	if (name === 'document')
		return {
			...
			notes: [
				...
				'FOR A REAL COVER PHOTO use cover.background: {"src":"assets/hero.jpg","size":"cover","position":"center","scrim":{"color":"#000000","opacity":0.35},"ink":"#ffffff"} — a full-bleed image on the cover sheet, edge to edge. backCover.background is the same shape and entirely independent. A PHOTO BEHIND TEXT NEEDS A "scrim" OR AN "ink", USUALLY BOTH: a dark photo swallows the near-black title, and theme.text cannot fix it (that token paints the WHOLE document, so a white cover title would come with white body text on white paper).',
	...
	if (name === 'theme')
		return {
			...
			notes: [
				...
				'TO SET COLORS, USE THE `theme` COMMAND — do not hand-write the config. `theme <file> --set \'{"preset":"forest","accent":"#0054fe"}\'` writes to the document\'s own envelope: a canvas\'s "document.theme", or — for a markdown file, which has no envelope — its COMPANION canvas, CREATED if it does not exist yet (the command names the file before writing it). `theme --all --set \'{...}\'` sets the workspace default in `skills-config.json`; `theme --save "Acme" --set \'{...}\'` stores a reusable named palette there; `theme --list` prints every preset and saved palette; `theme <file>` alone reports what a document is wearing and which file decides it. It validates first, and repaints an open browser.',
				...
				'WHERE A THEME LIVES, in one line: in the document\'s own envelope. A canvas keeps it in "document.theme". A markdown file has no envelope, so it keeps it in its COMPANION canvas — {"instantcanvas":1,"enhances":"report.md","document":{"theme":{...}},"blocks":[{"type":"markdown","src":"report.md"}]} — which is also where its cover and its running header go. Precedence, three levels: the document\'s own theme > the workspace default (`skills-config.json` "theme") > the built-in default. A canvas holding a form, a confirm or a sweep cannot carry a "document" at all (paper cannot submit), so it wears the workspace default and nothing else.',
			],
