...
const ENVELOPE = {
	...
	properties: {
		...
		createdWith: {
			type: 'string',
			required: true,
			description: 'The InstantCanvas version that created this canvas. Set by `npx instant-canvas stamp`, which reads it from the running CLI — do NOT write it by hand. It records the canvas\'s birth version so a future release can migrate it, and is never rewritten once present.',
			example: PKG_VERSION,
		},
