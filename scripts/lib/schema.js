...
const BLOCKS = {
	markdown: {
		...
		notes: [
			...
			'MATH just works — write LaTeX between $…$ or \\(…\\) inline, and $$…$$ or \\[…\\] as a centered block, in any markdown (inline "text", a "src" file, or a native .md). It is typeset server-side to themed, print-ready SVG, so you ship only the LaTeX. The standard TeX set is supported: fractions, sums, integrals, Greek, matrices via \\begin{pmatrix}…\\end{pmatrix}, aligned systems and cases. Guards: a $ next to a digit is a literal PRICE ($5, 5$), so write \\$ for a literal dollar and brace a formula that opens with a number; $ inside a code fence stays literal; invalid LaTeX degrades to a visible error showing the source, never a broken page.',
