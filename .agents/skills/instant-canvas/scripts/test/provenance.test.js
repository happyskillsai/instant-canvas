...
// The provenance stamp: `createdWith` records which skill version wrote a canvas,
...
// agent must not". A drift between the stamp and the running skill is NORMAL and
...
const canvas = (extra = {}) => ({ instantcanvas: 1, createdWith: SKILL_VERSION, title: 'T', blocks: [{ type: 'markdown', text: 'hi' }], ...extra })

// ---------------------------------------------------------------- the stamp is the program's, not the agent's

test('stamp writes the running skill version, read from skill.json — never a value an agent supplied', () => {
	const ws = workspace()
	...
	// The authority is skill.json, not the CLI, not the agent, not this test's guess.
	const declared = JSON.parse(fs.readFileSync(path.join(SKILL_ROOT, 'skill.json'), 'utf8')).version
...
test('stamp preserves the file byte for byte apart from the inserted line', () => {
	...
	assert.deepEqual(added, ['\t"createdWith": "' + SKILL_VERSION + '",'], 'exactly one new line, matching the file\'s indent and colon spacing')
	// Re-serializing instead of splicing once turned a one-line change into a
	// 148-line diff on a canvas the user owns.
	assert.equal(after.replace('\t"createdWith": "' + SKILL_VERSION + '",\n', ''), PRETTY)
})
...
test('stamp preserves a minified canvas as minified', () => {
	...
	run(['stamp', file, '--workspace', ws.dir])
	const after = fs.readFileSync(file, 'utf8')
	assert.equal(after, '{"instantcanvas":1,"createdWith":"' + SKILL_VERSION + '","title":"T","blocks":[]}')
...
test('a stamp that differs from the running skill is NOT a problem — drift is the normal case', () => {
	...
	assert.notEqual('99.999.999', SKILL_VERSION, 'the drift case is genuinely different from the runtime')
})
...
test('the envelope schema declares createdWith, so `catalog envelope` teaches it', () => {
	...
	assert.equal(ENVELOPE.example.createdWith, SKILL_VERSION)
	assert.match(ENVELOPE.properties.createdWith.description, /stamp/i)
})

// ---------------------------------------------------------------- one version, one source

test('the skill version has exactly one source: skill.json, read by lib/skillmeta', () => {
	const declared = JSON.parse(fs.readFileSync(path.join(SKILL_ROOT, 'skill.json'), 'utf8')).version
	assert.equal(SKILL_VERSION, declared)

	// A second hand-rolled read of skill.json would let the footer, /healthz and
	// the stamp drift apart. lib/skillmeta must be the only one that opens it.
	for (const f of ['instantcanvas.js', 'kernel.js']) {
		const src = fs.readFileSync(path.join(SKILL_ROOT, 'scripts', f), 'utf8')
		assert.ok(!/readFileSync\([^)]*skill\.json/.test(src), `${f} must read the version through lib/skillmeta, not by opening skill.json itself`)
		assert.match(src, /require\('\.\/lib\/skillmeta'\)/, `${f} reads the version from lib/skillmeta`)
	}
})

test('every canvas shipped inside the skill carries a stamp and validates', () => {
	// This is the test that would have caught the examples being left unstamped
	// when the field became required — they ship, and SKILL.md points agents at them.
	const dir = path.join(SKILL_ROOT, 'examples')
	...
	for (const f of files) {
		...
		const r = validate(raw, { root: SKILL_ROOT })
