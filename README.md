...
Death to the admin panel and BI tools: a local, schema-driven canvas runtime that lets coding agents render data visually (charts, tables, KPIs, markdown) and safely collect user input — forms, secrets, confirmations — in the user's browser, with values written straight to local files and **never entering the chat**.
...
```bash
npx happyskills install happyskillsai/instant-canvas
```

New to HappySkills? The [HappySkills docs](https://happyskills.ai/docs/) cover setting it up inside Claude Code and managing your skills.

**2 — Just ask.** Once the skill is installed, use Instant Canvas straight from a Claude Code prompt — no commands, no JSON, nothing to learn:

> Use Instant Canvas to generate a report from my markdown files, please add a beautiful front and back cover page using a free wallpaper website.

> Use Instant Canvas to visualize my data insights based on the latest data science analysis from this session.

> Use Instant Canvas to show me the latest images generated by Gemini Nano Banana Pro.

The agent wrangles your data into the canvas contract, runs the CLI, and your browser is already showing the result — themed, interactive, and yours to keep or discard.

---

**Using the CLI directly.** The skill drives the same open-source CLI you can run yourself — handy for a quick look, a script, or understanding what the agent is doing under the hood. Prerequisites: Node ≥ 20 (with npx), a desktop browser. Commands run from any directory — the current directory becomes the workspace:
