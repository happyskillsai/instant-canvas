...
2. **The HappySkills skill** at `.agents/skills/instant-canvas/` — the ~55 KB agent-facing contract. Its `skill.json` version is mirrored from `package.json` in the same release commit, and the bundle is republished manually through the `happyskills-publish` flow.
