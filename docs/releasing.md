...
1. **The npm package `instant-canvas`** — the runtime. Published manually with `npm publish` from the repo root; the `prepublishOnly` script re-runs the full test suite first, and publishing requires `npm login`.
2. **The HappySkills skill** at `.agents/skills/instant-canvas/` — the ~20 KB agent-facing contract. Its `skill.json` version is mirrored from `package.json` in the same release commit, and the bundle is republished manually through the `happyskills-publish` flow.
...
2. Smoke the consumer path from any directory: `npx -y instant-canvas catalog`.
