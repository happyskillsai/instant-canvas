...
The token reaches the browser via `__IC_TOKEN__` placeholder substitution when the shell is served (CSP forbids inline scripts, so it cannot be injected as a `<script>` variable). Asset URLs carry it as a query parameter — except the vendored fonts, which are served tokenless (a CSS `url()` cannot append the token) and are exempt at the gate. `serveShell` substitutes more placeholders the same way: `__IC_VERSION__` (the footer version) and the three extension unions `__IC_IMAGE_EXTS__` / `__IC_VIDEO_EXTS__` / `__IC_AUDIO_EXTS__` (`GALLERY_IMAGE_EXTS` / `MEDIA_VIDEO_EXTS` / `MEDIA_AUDIO_EXTS` from `lib/gallery.js`, landed in `<body data-image-exts data-video-exts data-audio-exts>`) — the latter let the browser classify a routed `#/c/` path as an image, a video or audio without a copied list (see [frontend.md](frontend.md)).
