...
`companionIndex(root)` walks the same 2-level depth the scan does, reads every `*.json` through the shared `lib/canvasfile.js` marker check, and maps *document → the canvas that enhances it*. Three consumers, one index:
