...
- **markdown** via vendored markdown-it (`html: false, linkify: true`); `src` content and every workspace-local image arrive **pre-inlined by the kernel** (`data:` URIs — the browser never issues a request, and `img-src 'self' data:` already permits them). Oversize, unreadable, or out-of-workspace images degrade to a labeled fallback, never a broken image. Fenced code is highlighted by vendored **highlight.js** (full build, 192 grammars) through markdown-it's `highlight` hook; only a *declared* language is highlighted, since auto-detection across 192 grammars mislabels short snippets. Two skill-side markdown-it core rules fill gaps the library leaves: GFM **task lists**, and rewriting markdown-it's `style="text-align:…"` column alignment into a **class** (see [gotchas/frontend.md](gotchas/frontend.md)).
