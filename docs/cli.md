...
2. **Validate locally first.** An invalid canvas never launches the UI; the CLI exits 1 with the full `errors[]` array. A **markdown file — or a folder — skips this step entirely** — there is no envelope to check, because the runtime synthesises it (a `.md` becomes a markdown block, a directory a gallery block; see [canvas-schema.md](canvas-schema.md)).
