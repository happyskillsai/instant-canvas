...
**The same lesson had to be learned a second time, one layer down, because the confirming ping had no tolerance of its own.** `readAlive()` health-pings with a 500 ms deadline and, on **any** failure, deletes the registry entry. `pingHealth` collapsed every outcome into a bare `null`, so a kernel that was alive and merely slow to answer was indistinguishable from one that had died — and the punishment was not a false negative but an **eviction**: the entry was removed, so the next lookup could not find the kernel either, and a live process became unreachable. It surfaced as an intermittent `e2e` failure where `status` reported `running: false` moments after `open` had returned `opened`, and only under the extra load of a coverage run.

The fix rests on a distinction the transport already makes and the code was throwing away: **a dead port is REFUSED (microseconds), a busy one TIMES OUT.** `probeHealth()` now reports which, and only a timeout earns a second, longer probe; a refusal is reaped on the first pass exactly as before, so `kill -9` recovery keeps its speed (measured: 7 ms to reap a dead port, versus a 2 s retry budget it never touches). The general rule, now stated twice in this file: **a deadline that expires is evidence about the clock, not about the peer** — and any code that *destroys state* on a failed probe must be certain the probe measured the peer.
