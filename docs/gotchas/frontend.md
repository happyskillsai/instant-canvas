...
## The click that OPENS a popover will also close it, unless the anchor counts as "inside"

The fifth member of the outside-click family, and it arrives from the opposite direction to the four above: nothing re-renders, nothing detaches, and the popover still shuts instantly. A menu opened from a **button** (the browse toolbar's ⋮) is created *during that button's own click*, in the bubble phase. The document-level "close on an outside click" listener runs later in the same event, finds a menu that is now open, and asks where the click landed — on the button, which is not inside the menu. So it closes what the click just opened. The button reads as dead: press it, nothing appears, and there is no error to explain why.

The fix is the one the palette panel already encodes and is easy to under-copy: "inside" means **the menu OR the anchor that opens it** (`e.target.closest('#palettePanel') || e.target.closest('#paletteBtn')`). `openContextMenu` therefore records its `anchorEl` and the outside-click handler returns early when the click landed within it. Note this is *not* the detached-node problem — capture vs bubble makes no difference here, because the anchor is a perfectly ordinary attached node. Two different bugs share one listener, and fixing one does not fix the other: **capture phase answers "was the target still in the tree?", the anchor check answers "is this the click that opened me?"**

A right-click anchor never hits this (a `contextmenu` event fires no `click`), which is exactly why it survives casual testing — the feature works from the keyboard-free path a developer reaches for first, and only the discoverability button is broken. Assert it by *clicking the button in a real browser and counting the menu items*, not by checking that the handler ran.
...
The browse view's type filter hit this from a third angle, and it sharpens the rule into *one explicit rule per SURFACE, not per element*. Making the filter hide tiles rather than rebuild them meant `.gt` had to answer to `hidden` in **two layouts**, and each needed its own rule: `.gt[hidden]` for the grid (where `.bt-card{display:flex}` on a canvas/document/folder card outranks the UA), and `.gallery.g-list .gt[hidden]` for list mode (where `.gallery.g-list .gt{display:grid}` outranks *both*). The trap is that the tile types are not equally affected — **`.gt` itself declares no `display`**, so an *image* tile is hidden by the UA rule with or without ours, while a *card* tile computes `display:flex` and sits there at a full 176×176 px. Both were measured in a real browser. The consequence for testing is the sharper half and lives in [gotchas/testing.md](testing.md): a `[hidden]` assertion aimed at the element that needs no help **cannot fail**.

## Hiding an element stops the work the BROWSER owns, never the work you kicked off yourself

`display: none` is a wonderfully cheap way to stop work — a hidden tile's `<img loading="lazy">` never intersects the viewport, so it never fetches. The browse filter leans on exactly that: filtering to Canvases in an image-heavy folder actively *stops* image loading rather than merely hiding it (measured: navigating into a 12-image folder with the filter on fetches **0**; clearing it fetches exactly 12). The mistake is generalising that to everything a tile "does".

Video posters are captured by `grabPoster`, which builds an **off-DOM** `<video>` and never appends it — deliberately, so a capture cannot perturb layout. But an element outside the document is outside the reach of *any* CSS: its tile's `display: none` means nothing to it. So when the filter changed from "don't build the tile" to "build it and hide it", every filtered-out video started fetching and decoding a frame **while invisible** — measured at six hidden tiles issuing six requests, with none left to make once revealed. All of the cost, none of the benefit, and completely silent: the grid looked right.

The fix is to gate the capture on the same predicate that decides visibility and defer it to the first reveal. Two supports make that safe: `mountVideoPoster` no-ops once a poster has landed, and `capturePoster` caches the *promise* per `(rel, mtime)` — including a **rejection, cached as `null`** — so a permanently-failing video cannot turn repeated reveals into a retry storm.

The rule generalises past posters: **when you switch a surface from "not built" to "built but hidden", audit every side effect the build kicks off**, and ask of each whether it is the browser's work (gated by CSS) or yours (gated only by you). Timers, observers, `fetch`, an `Audio`/`Image`/`Video` constructed in script, a `requestIdleCallback` — none of them can see a `display` property. Assert it by **counting real resource timings**, not by reading the code: `performance.getEntriesByType('resource')` is the only witness that a hidden thing stayed quiet.
...
## A document-level `paste` handler must YIELD before it PREVENTS — it sits under every input in the app

The Esc/arrow rule above, one event family over, and the stakes are higher: a `keydown` handler that pre-empts a sub-surface steals one key, while a `paste` handler that calls `preventDefault()` too early breaks **text entry everywhere at once** — the native `.env` form's `KEY=value` paste, `⌘K`'s search box, every field in every form — with no error and no console warning. The gesture the reader performs is identical in all of them; only the surface differs.

So the order is load-bearing: **read the clipboard, decide it is ours, and only then `preventDefault()`**. The paste handler yields when focus (or the event's own target) is in an `input`/`textarea`/`select`/`contentEditable`/any `form`, when another surface owns the keyboard (the item overlay, presenting, `⌘K`, the palette panel, the folder context menu, a `.g-modal`, a `.gallery.g-selecting`, `body.nav-open`), or when the route is not a browse view — the same enumeration the overlay's keyboard block makes, kept deliberately in step with it. **Both** the focused element and `e.target` are tested, not just `document.activeElement`: an event dispatched at an element is targeted there whether or not focus followed, which is exactly the shape a test synthesizes.

The second half is quieter and is a *product* rule rather than a mechanical one: **a clipboard carrying nothing we want is a silent no-op.** No toast, no request. The tempting "nothing to paste here" message fires on every stray `⌘V` over prose, which is most of them — an error that common reads as the feature being broken.

## Two guards that both protect one surface are each individually unfalsifiable

Discovered while sabotage-verifying the rule above, and worth recording because the *conclusion* is not "delete one". The `.env` form is reached at `#/c/.env`, which renders it **inside the item overlay** — so a file paste into an env field is turned away twice: once by the overlay yield, once by the focus-in-field yield. Remove either alone and every test stays green. The natural readings are both wrong: "the guard is untested" (it is — jointly) and "the redundancy is dead code" (it is not — each covers surfaces the other does not, and a future field on the browse pane itself would have only the field yield standing).

What it actually demands is that the **sabotage be proportional to the redundancy**: break the guards *together* and watch the assertion go red, then reason about which one carries which surface. Sabotaging one at a time and seeing green proves nothing about either. The general shape — the sibling of "an assertion that something did NOT happen is vacuous unless it COULD have happened" ([testing.md](testing.md)) — is that **defence in depth makes each individual layer unfalsifiable by construction**, so a suite can only ever pin the depth, never the layers.
...
## A pixel offset that dodges a sibling panel breaks when the panel stops being a sibling

The image stage's zoom bar (`.g-zoombar`) is `position: absolute` inside `.img-stage`, centered with `left: calc(50% - 157px); transform: translateX(-50%)`. The `-157px` is not a magic number — it is half the 300px `.g-meta` metadata panel, so the bar centers within the *stage column* that sits to the **left** of that panel. Correct, in the gallery block's detail modal, where `.g-meta` is a flex **sibling** of the stage and genuinely eats 300px on the right.
...
## `navigator.canShare()` does NOT check the file-type allow-list it is asked about

The Web Share API looks like it comes with its own feature test — `canShare({files})` reads exactly like "will this browser share these files". It is not. Blink's `CanShareInternal()` validates **structure** only: is there a files array, is it non-empty, does the URL parse. Chromium's actual allow-list — 41 extensions and 28 MIME types in `chrome/browser/webshare/share_service_impl.cc` — is enforced later, in the **browser process**, at `share()` time.

So `canShare()` returns **true** for `.mov`, `.heic` and `.zip`, and `share()` then rejects them. Measured on `http://127.0.0.1` in Chrome 151: all three pass the probe. A Share button gated on `canShare` alone therefore looks available, is clicked, and fails *after* the gesture — the worst possible ordering, because the reader has already committed and the failure reads as the feature being broken rather than the file being unsupported.

The two absences that matter most are the two a Mac user hits first: **`.mov` and `.heic`** are iPhone video and iPhone photos. The fix is to keep our own extension screen client-side (`WEBSHARE_EXTS` in `app.js`) and only offer the row when it passes. Two supports make that honest rather than a second source of truth: the list is a documented intersection with what `/api/gallery/file` will actually stream (it serves streamable extensions only, so the two gates agree by construction), and the real file is re-checked with `canShare({files:[file]})` just before `share()`, because the capability probe answered for a synthetic PNG.
